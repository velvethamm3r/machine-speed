...
### Run 2026-08-17 (daily — coverage extended to Aug 17; three capability items, one new watchlist thread)

Coverage moved from Aug 14 to Aug 17; `coverageStart` unchanged at Jul 1 (grow mode). Three items added (board now 80), all in Capability. The fresh window (Aug 15–17) was quiet; Policy, Defense, Attacks and Markets returned only already-shown or out-of-window material and were left unchanged rather than padded. All three additions are new-to-board catches inside the stated period rather than Aug 15–17 events.

**New this run (3) — all Capability**
...
- HAWK withdrawn from NIST post-quantum signature standardization (Jul 29, one day after Anthropic’s Mythos halved its effective security); NIST candidate page now lists it withdrawn · 2026-07-29 · press (via The Hacker News) [NIST / pqc-forum primary unfetchable — upgrade to on-record next run]

**Sourcing / judgment calls**
- GLM-5.3 and HAWK carried at press because their primaries (z.ai, NIST) required a fetch approval unavailable in this unattended run. Both flagged for upgrade.
...
- HAWK withdrawal filed as a separate Capability item from the Jul 28 Mythos cryptanalysis item (`anthropic-mythos-cryptanalysis-hawk-aes`): distinct facts (the cryptanalytic finding vs. the standards-process withdrawal), and the single-URL item model can’t fold the withdrawal in without downgrading the finding from its anthropic.com on-record source. The “first ever” framing some outlets used was not asserted.
...
**Strip / isNew** — every prior run’s isNew flag was cleared. The three additions carry isNew:true (3 in strip). No board item falls in the Aug 15–17 auto-window, so the strip is exactly this run’s three.

**Watchlist** — new thread “AI cryptanalysis of cryptographic standards” opened (Mythos vs HAWK and 7-round AES; HAWK’s NIST withdrawal; → 2026-07-29). “Vendor benchmark claims” updated with GLM-5.3’s self-reported jumps and ZOOMSDAY (→ 2026-08-14). “Open-weight cyber gap” left unchanged — GLM-5.3’s open-weight status was not confirmed in the source opened, so it was not folded into that thread. All other threads carried forward unchanged.
...
_Synced with data.json as of the 2026-08-17 run._
...
- AI cryptanalysis of cryptographic standards · Anthropic's Mythos system found a lattice automorphism roughly halving the effective security of the HAWK post-quantum signature scheme (July 28) and a shortcut speeding the strongest known attack on a 7-round test version of AES by 200 to 800 times; one day later HAWK's submission team withdrew the scheme from NIST's additional-signatures standardization process and NIST's candidate page now lists it as withdrawn. Both cryptanalytic results are theoretical and touch no deployed system, but the withdrawal is the first concrete standards consequence. Watching whether AI cryptanalysis reaches other candidates or fielded schemes. · 2026-07-29
