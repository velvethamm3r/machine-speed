...
### Run 2026-08-14 (daily — coverage extended to Aug 14; five new items across three passes)

Coverage moved from Aug 13 to Aug 14; `coverageStart` unchanged at Jul 1 (grow mode). Five items added (board now 76). The first pass found one genuinely new in-window item (Taiwan) and read the rest of the day as quiet; on Daria's follow-up prompt to sweep harder (mirroring the Aug 12 second-pass precedent), a deeper pass surfaced three more new-to-board, in-window catches; and Daria then shared the California governor's announcement, which became the fifth. Each item is verified against a primary that was opened. Markets returned nothing board-worthy dated in the window.
...
**Strip / isNew** — every prior run's `isNew` flag was cleared. The five additions (Taiwan, Rapid7 SharePoint, Google ADK, Trellix, California AI Cyber Defense Program) carry `isNew:true` (5 in strip, within the 6 cap). The Aug 12 White House memo, flagged last run, is dated within the 2-day auto-window and so was set `isNew:false` to hold it out; the Aug 10–11 items (Casar push, Sanders letter, reasoning-trace) fall outside the auto-window and needed no explicit flag.
...
- Vendor benchmark claims · Microsoft claims ~96% on CyberGym; Sakana 86.9%; Meta 92.9% pass@1 on Cybench for Muse Spark 1.1 — none independently reproduced. Axios reports red-teamers finding public cyber benchmarks saturated within four weeks of release, and SecRespond finds no frontier model completes detection and remediation on any post-compromise range. Off-by-1 Labs adds that only about one in four AI-generated patches actually fixes the CVE, while PortSwigger's HTTP Terminator showed an AI-assisted loop inventing novel attack techniques with a human still needed for the hardest results. VulnCheck's first-half 2026 report adds a check from the other direction: of 1,061 vulnerabilities attributed to AI-assisted discovery, only about 1.3% were confirmed exploited in the wild, the same rate as any other — the measurement layer remains the weak point in both directions. Rapid7's Aug 11 SharePoint co-disclosure carries the same caveat from the offensive side: an agentic workflow ran ~80,000 tool calls over 24 days to help chain two flaws to unauthenticated RCE, but the firm says a fully automated approach would not have worked and expert steering was essential. · 2026-08-11
