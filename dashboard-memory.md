...
### Run 2026-08-12 (daily — coverage extended to Aug 12; one new item + three sourcing upgrades)

Coverage moved from Aug 11 to Aug 12; `coverageStart` unchanged at Jul 1 (grow mode). One item added (board now 66). Policy and Markets returned nothing verifiable in the fresh window and were left empty rather than padded.
...
**Strip / isNew** — every prior run's `isNew` flag was cleared. Only the gym item carries `isNew:true` (1 in strip). Daybreak (now the only other Aug 10-dated item) was set `isNew:false` to hold it out, since it was added last run and only its sourcing changed today.

**Watchlist** — 'Agent-abuse attack surface' updated (seven → eight routes, adding the in-the-wild OpenClaw overstep; → 2026-08-10) and 'Gated model access' re-synced to the corrected Daybreak/Astra facts (→ 2026-08-10). All other threads carried forward unchanged.

_Prune note: nothing aged past the ~5-week floor this run (oldest board item is 2026-07-01, the coverage start). Board items persist within the stated period, so shown-item log entries are retained for dedup rather than dropped._

## Watchlist (thread · current status · last-changed)

_Synced with data.json as of the 2026-08-12 run._
