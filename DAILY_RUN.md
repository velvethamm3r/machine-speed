...
where the full history belongs and where it costs nothing. Target roughly 600–900
characters; past about 1,200 the thread has stopped being a status and needs either a trim
or a split into two threads. Retire rather than accumulate: a storyline that has stopped
moving gets an honest closing status ("Resolved", "Dormant"), not another appended clause.
