# Machine Speed — Aug 11, 2026

*A daily, source-verified intelligence board on frontier AI cyber capability and the defense & policy lag around it.* Covering Jul 1 – Aug 11, 2026 · 65 items. [Live board](https://machinespeed.techpointe.org/) · [RSS](https://machinespeed.techpointe.org/feed.xml)

---

## New to the board

- **Capability** — OpenAI delays its unreleased Astra model after safety testing could not rule out a 'Critical' cyber capability. [OpenAI (via Axios)](https://www.axios.com/2026/08/10/openai-gpt-astra-restrictions-safety-hacking-defenders)
- **Defense** — OpenAI launches Daybreak, gating a cyber-tuned GPT-5.6 model to vetted security partners. [OpenAI (via Help Net Security)](https://www.helpnetsecurity.com/2026/08/11/openai-daybreak-cyber-models/)
- **Markets** — IBM's 2026 breach report puts one in four malicious breaches as AI-enabled, at about $6 million each. [IBM Security](https://newsroom.ibm.com/2026-07-29-ibm-study-one-in-four-malicious-breaches-are-ai-enabled,-costing-companies-6-million-on-average)
- **Capability** — Anthropic says its Mythos system found new mathematical weaknesses in the Hawk post-quantum scheme and reduced-round AES. [Anthropic (via CyberScoop)](https://cyberscoop.com/anthropic-claude-mythos-encryption-flaws-hawk-aes-pqc/)
- **Capability** — VulnCheck finds AI-discovered vulnerabilities are exploited in the wild at the same low rate as any other. [VulnCheck](https://www.vulncheck.com/blog/state-of-exploitation-1h-2026)

## Capability

**OpenAI delays its unreleased Astra model after safety testing could not rule out a 'Critical' cyber capability**  
OpenAI said it is holding back Astra, an unreleased model it describes as advanced at agentic coding and cybersecurity, after preliminary safety evaluations indicated it could not rule out a 'Critical' cyber capability under its Preparedness Framework — the first time OpenAI has flagged that top threshold, whose definition covers developing functional zero-day exploits or executing end-to-end attacks against hardened targets without human intervention. OpenAI delayed the release and applied additional security controls.  
*Reported by press — [OpenAI (via Axios)](https://www.axios.com/2026/08/10/openai-gpt-astra-restrictions-safety-hacking-defenders), Aug 10, 2026*
...
Anthropic reported that its Claude-based Mythos system identified a lattice automorphism that roughly halves the effective security of the Hawk post-quantum signature scheme — meaning its key sizes would need to double — and a shortcut that makes the strongest known theoretical attack on a 7-round test version of AES 200 to 800 times faster. Both results are theoretical: the AES work does not touch the full 10-round cipher in deployed software, and cryptographers reviewing the results note neither flaw affects systems now in use.  
*Reported by press — [Anthropic (via CyberScoop)](https://cyberscoop.com/anthropic-claude-mythos-encryption-flaws-hawk-aes-pqc/), Jul 28, 2026*
...
**OpenAI launches Daybreak, gating a cyber-tuned GPT-5.6 model to vetted security partners**  
OpenAI expanded its Daybreak program into two partner-only tiers: Blue, giving approved defenders access to GPT-5.6 Sol without system-level cyber guardrails, and Red, carrying a new GPT-5.6-Cyber model rated 'High' cyber capability, for exploit validation, red-teaming and vulnerability research. Access is limited to vetted firms — reported to include Accenture, IBM, CrowdStrike, Cisco and Palo Alto Networks among a cohort of roughly sixteen — who deliver findings and services to clients rather than the models themselves.  
*Reported by press — [OpenAI (via Help Net Security)](https://www.helpnetsecurity.com/2026/08/11/openai-daybreak-cyber-models/), Aug 10, 2026*
...
- **Gated model access** — Google still restricts Gemini 3.5 Flash Cyber to governments and trusted partners, and OpenAI's August 10 Daybreak expansion now gates a cyber-tuned GPT-5.6-Cyber model, rated 'High' capability, to roughly sixteen vetted security firms that receive findings and services rather than the models themselves. OpenAI separately delayed its unreleased Astra model after safety testing could not rule out a 'Critical' cyber capability. Both moves keep the most capable cyber models out of general release, under gating or held back entirely. CISA is reportedly running Anthropic's Mythos over federal agency code. *(last changed Aug 10, 2026)*
...
Coverage extended to August 11, 2026; coverageStart is unchanged at July 1 (the board remains in its stated-period 'grow' mode, as it has since August 5 — switching to a rolling window that drops older items would change what leaves the live board and is left for Daria to confirm). Five items were added: three in Capability, one in Defense, one in Markets. Policy and Attacks returned nothing verifiable in the fresh window and were left as they were rather than padded.

Fresh this run (Aug 10). OpenAI announced two distinct things the same day, filed separately: the Daybreak expansion — a partner-only gating of a cyber-tuned GPT-5.6-Cyber model (Defense) — and its decision to delay the unreleased Astra model after safety testing could not rule out a 'Critical' cyber capability under its Preparedness Framework (Capability). Both are first-party disclosures, but OpenAI's own announcement pages could not be fetched in this unattended run (the fetch needs an approval no one is present to give), so both are carried at press confidence against multiple corroborating outlets — Axios, Help Net Security, TechCrunch, CNBC and The Hacker News, all consistent on the names, tiers and partner list. On a later run, re-open openai.com and upgrade: the Daybreak launch to self-reported (a vendor capability release, like the Gemini 3.5 Flash Cyber item) and the Astra 'cannot rule out Critical' disclosure to on-record (a first-party framework statement, like the Meta Muse Spark item).

Late catches (late July, missed by earlier runs, inside the window). Anthropic's July 28 report that its Mythos system found weaknesses in the Hawk post-quantum scheme and a faster attack on 7-round AES is carried at press via CyberScoop because anthropic.com could not be fetched in this unattended run; the figures (Hawk security roughly halved; the strongest known 7-round-AES attack 200–800x faster) and the caveats (both purely theoretical, neither touching deployed full-round AES or systems in use) are corroborated by cryptographer Matthew Green's analysis — upgrade to the Anthropic primary next run. VulnCheck's State of Exploitation report for H1 2026 (cited to VulnCheck's own page) found 14 of 1,061 AI-attributed vulnerabilities exploited in the wild, about 1.3%, matching the overall rate; its separate figure for an Anthropic 'Project Glasswing' program was omitted because that name conflicts with this board's established 'Mythos' naming for Anthropic's tooling and could not be reconciled against a source — the detail was dropped, not guessed. IBM's 2026 Cost of a Data Breach report (IBM newsroom, opened) is filed in Markets as a capital/cost-of-risk measurement — a one-in-four AI-enabled share and a ~$6M average cost is how the money is now sizing AI-driven breaches, which is the closest fit among the five lanes.

Checked and excluded. The Five Eyes joint warning that capable AI hacking models are 'months away' is dated June 24, 2026 — before the coverage window — so it is left off the board rather than backdated. Today is a Microsoft Patch Tuesday, but an AI-attributed angle on it was not yet separately sourced at run time and no item was written on it. The GTIG AI Threat Tracker surfaced in searches is dated May 11 and out of window.

Strip. The five items added this run carry isNew so they surface under the board's 'New to the board' strip (five, within the six-item cap); every prior run's isNew flag was cleared so the strip reflects this run alone. The three late-July items are days older than the strip's 48-hour date rule but are genuinely new to the board, which is what isNew is for.

Briefs. Neither open brief — the OpenAI–Hugging Face eval breach and 'AI reaches the water sector' — produced a separately sourced development this run, so both are carried forward unchanged. The Astra and Daybreak news is OpenAI cyber activity but not part of the Hugging Face incident's own storyline, so it was filed as board items rather than folded into that brief.

**Change since previous run**

Coverage extended to August 11 with five items added (board now 65): OpenAI's August 10 Daybreak cyber-model gating and its delay of the 'Critical'-rated Astra model, plus three late-caught late-July items — Anthropic's Mythos cryptanalysis, VulnCheck's AI-exploitation-rate finding and IBM's AI-enabled breach-cost figures — while the policy and attacks lanes added no standalone items.
