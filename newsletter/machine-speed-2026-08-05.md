...
This run added the Markets lane and the first incident thread, and both required calls worth recording. On Markets: the insurance material that best explains the structure of the problem — Munich Re's 2026 trends report, Aon's AI risk agenda, the January 2026 ISO generative-AI exclusion, CFC's affirmative AI products, Lloyd's market message — all predates July 1 and so sits outside the stated coverage period. Rather than backdate the period or smuggle those in under later dates, they are carried as a watchlist thread and the lane holds only the five items that genuinely fall inside it. The AIUC figures (90% silent cover, $100bn projected losses) are labelled vendor rather than researchers: AIUC sells AI insurance, the primary report was not obtainable, and both numbers appear only in secondary coverage. The NAIC item is included with its scope stated plainly, because the meeting concerns insurers' own use of AI in underwriting and is easy to misread as a regulatory response to AI-driven cyber loss.
