...
- **Capability** — Off-by-1 Labs: about three in four AI-generated vulnerability patches are broken or incomplete. [Off-by-1 Labs (1Password)](https://1password.com/files/resources/frontier-models-vulnerability-patches-flawed.pdf)
...
- **Defense** — OWASP publishes the 2026 LLM Top 10, blending expert judgement with real-incident data. [OWASP GenAI Security Project](https://genai.owasp.org/resource/owasp-genai-llm-top-10-2026/)
- **Attacks** — Okta documents gray-market services reselling frontier-model access — and reading every prompt that passes through. [Okta Threat Intelligence](https://www.okta.com/blog/threat-intelligence/free_tokens_for_sale/)
- **Attacks** — CrowdStrike's 2026 Threat Hunting Report says AI is now embedded across adversary operations. [CrowdStrike](https://www.crowdstrike.com/en-us/press-releases/crowdstrike-2026-threat-hunting-report/)
...
Coverage extended to August 10, 2026; coverageStart is unchanged at July 1. Five items were added — two in Capability, one in Defense, two in Attacks. Neither Policy nor Markets returned anything verifiable in the window, and both were left as they were rather than padded.

Sourcing. Every added item is cited to a primary source that was opened and read: PortSwigger's own write-up, the Off-by-1 Labs (1Password) report PDF, the OWASP GenAI project page, Okta's threat-intelligence post and CrowdStrike's report. Two dates were corrected against those primaries: the OWASP 2026 Top 10 is dated August 4 — OWASP's own project page states 'published August 4, 2026' — and Okta's report is dated August 4; press coverage of both only appeared on August 6. The Off-by-1 Labs PDF carries no explicit publication date, so the August 6 date of its first public reporting is used. The OWASP item states only what the primary and Infosecurity Magazine corroborate; a '6,639 incidents / 75-25 weighting' figure that appeared in secondary coverage is left off.

Omitted. PortSwigger attributes its Apache Traffic Server finding to CVE-2026-63078, but The Hacker News checked on August 7 and found no record of that identifier in CVE.org or NVD, and it is absent from Apache's July advisory — so the number is left off the board and only the fact of the zero-day is stated. A single sensational-press claim that a Novee Security demo achieved a full Microsoft Copilot session takeover across three vendors was held for want of a primary or a second established source. DEF CON 34 coverage produced schedules and thematic recaps but no separately sourced finding.

Strip. The five items added this run carry isNew so they appear under the board's 'New to the board' strip; older items are left to the date rule.

Brief. A brief, 'AI reaches the water sector', was opened around Dragos's May 6 account of commercial LLMs (Claude as the technical workhorse, OpenAI's models in support) used to run a Monterrey water-utility intrusion — a genuine AI-cyber milestone that predates the coverage window, which is why it lives in a brief rather than on the board. The July US water attacks and New York's August 3 grant announcement are included as the sector backdrop, each carrying a note that its own sources do not invoke AI; the AI thread is the Monterrey intrusion alone, where Dragos found the OT attempts failed and no control systems were accessed.

Brief expansion. The OpenAI–Hugging Face brief gained two panels: an Anthropic-disclosure panel (its July 30 account — the July 23 halt, the 'real company must be part of the exercise' rationalization, Mythos 5's PyPI package, and the 'closer to a harness and operational failure than a model alignment failure' characterization, all quoted from Anthropic's own post) and an Insurance panel (the coverage-trigger gap, the silent-to-affirmative shift, Delinea's 42% exclusions figure and the claim-frequency argument). An 'Acrisure London Wholesale' attribution that appeared in a working draft was dropped: it is in none of the opened sources, and the 20–50% premium-reduction figure is carried to FinTech Global, where it actually appears.

Earlier arc. Panel 1 was extended back to the May origin using OpenAI's August 5 Black Hat talk as reported by SC Media and transcribed by Simon Willison (carried at press, since the account came from the talk rather than an OpenAI document I opened). Two details from the working-draft images were corrected against that reporting: May 26 was the first SSRF exploit of Artifactory, not a zero-day — the zero-day RCE was June 26 — and July 4 was the outage and credential revocation, but OpenAI did not connect the activity to its own agents until July 20. Both corrected dates are stated on the face of the stages.

**Change since previous run**

Coverage moved to August 10 with five primary-sourced items added and a new brief opened — 'AI reaches the water sector', anchored on Dragos's account of Claude and GPT models used to run a Monterrey water-utility intrusion — while the policy and markets lanes added no standalone items.
