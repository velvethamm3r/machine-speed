...
### Run 2026-08-25 (daily — coverage extended to Aug 25; three items added; two sweep leads found already on the board)

Coverage moved from Aug 24 to Aug 25; `coverageStart` unchanged at Jul 1 (grow mode). Three items added (board now 94). The fresh window since the Aug 24 runs was short; the additions are Aug 20–21 developments the Aug 24 passes did not carry, all cited to primaries opened this run. Policy, Attacks and Markets returned nothing new-and-verifiable dated in the fresh window and were left unchanged rather than padded.

**New this run (3)**
- Anthropic widens defender access to Claude Mythos 5 by shipping defined outputs (a vulnerability patch or a security alert through partner tools; Claude Security scans for Enterprise) rather than raw model access, launches a $35M "Defender Advantage Fund" (0xDAF) in Claude credits for open-source security patching/automation, and expands its Cyber Verification Program (reduced safeguards on Opus/Sonnet for vetted defenders) · 2026-08-21 · Defense · on-record · [claude.com/blog, opened]
- Aikido Security benchmark: ten models run three times each against 32 fresh CVEs in a bounded, internet-free harness with frozen prompts; open-weight models matched or beat closed frontier on pooled pass@3 recall — DeepSeek V4 Pro 28/32, ahead of Claude Opus 5 and Grok 4.6 at 26/32 — at roughly half the cost (~$295 for three DeepSeek Pro runs vs ~$450–590 per frontier pass) · 2026-08-21 · Capability · self-reported · [aikido.dev/blog, opened]
- UK NCSC interim guidance on managing the cyber risk of agentic AI — seven considerations (failure-scenario threat modelling, permitted/prohibited actions, human-oversight levels, sandboxing, logging/monitoring, attribution, and an emergency shutdown to "pull the plug"), stated as interim advice that formal guidance will supersede · 2026-08-20 · Defense · on-record · [ncsc.gov.uk/blogs, opened]

**Sourcing / judgment calls**
- Anthropic filed `on-record` (a formal first-party program announcement) in Defense; the item's core rests on the program, the $35M fund and the Cyber Verification Program changes, not on the "most capable model" phrasing. "Defender Advantage" was absent from the board; Mythos 5 is referenced elsewhere (the Anthropic eval disclosure, the AISI report, the GLM-5.3 comparison) but this Aug 21 defenders program was new.
- Aikido filed `self-reported`, matching the **XBOW precedent** (`xbow-cross-model-offensive-security-cost-comparison`) for a security vendor publishing a cross-model benchmark run on its own harness that no independent party has reproduced — not `researchers`, which the board reserves for academic/independent-lab benchmarks (SecRespond, Off-by-1). Recall scores and costs quoted exactly from Aikido's post; "frontier" is the benchmarker's framing.
- The NCSC Aug 20 guidance is kept **distinct** from the existing Jul 7 `uk-ncsc-cyber-shield-agentic-defence` (pol) item: Cyber Shield is a national agentic-*defence programme* (agency initiative → pol); this is defensive *deployment guidance* for organisations (guidance/mitigations → def). Different post, different URL, different date.

**Sweep leads found ALREADY ON THE BOARD (deduped, not re-added)**
- White House "Expanding Capabilities to Combat Transnational Cyber-Enabled Crime" memo (Aug 12) — already `whitehouse-transnational-cyber-crime-private-sector-operations-memo`.
- Resilience H1 2026 claims report (zero AI-vector losses; 85.3% social engineering; Jul 30) — already `resilience-h1-2026-claims`.

**Checked and held (below significance bar or off-remit)**
- Zimbra RCE CVE-2026-73570 added to CISA KEV Aug 21 (CVSS 8.9, GHSA-jqh7-pchh-v74j; NVD fetch timed out, verified via GitHub Advisory Database) — a routine single-product KEV addition with no AI angle; below the board's significance bar for a pure-cyber item.
- NIST SP 1353 ipd, QuickStart guide for using AI in CSF analysis/reporting (Aug 19, comment draft, comments due Oct 15) — a comment-stage draft, marginal.
- California AB 1651 (Aug 22, State Bar of California AI) — no cybersecurity provisions; off-remit.
- Out of window / already held: Meta "rogue model" 4th disclosure (Aug 5–6), GhostSplice MCP attack (Jul), TrueConf/VMware/Windows AFD KEV adds (Aug 17–20), Context7 MCP CVE-2026-75130 (Aug 18), various vendor product launches.

**Strip / isNew** — the three prior `isNew` flags (Adversa Grok/Gemini, state-AG demand, Guidelight) were cleared; the three additions carry `isNew:true` (3 in strip, within the 6 cap). Build exited 0 with no warnings.

**Watchlist** — three threads updated to Aug 21: "Vendor benchmark claims" (Aikido added as an independent-side benchmark showing open-weight parity at lower cost), "Open-weight cyber gap" (Aikido complicates the gap from the capability side), and "Gated model access" (Anthropic's outputs-not-access model + $35M fund + expanded Cyber Verification Program). The NCSC guidance opened no thread — filed as a standalone defensive-guidance board item, matching how single guidance/disclosure items are handled. No new thread; briefs unchanged (neither the Hugging Face nor the water-sector storyline moved).

_Prune note: nothing aged past the ~5-week floor this run (oldest board item is 2026-07-01, the coverage start)._

## Watchlist (thread · current status · last-changed)

_Synced with data.json as of the 2026-08-25 run._
